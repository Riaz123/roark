...
public class KeywordSelector {
	...
	private static KeywordSelector instance;
	KeywordSelector(){
		
	} 
	public static synchronized KeywordSelector getInstance(){
	  if (instance == null)
	  {
	  instance = new KeywordSelector();
	  }
	  return instance;
	  }
	...
	public void executeKeyword(TestSuite testSuite, TestcaseStep testcaseStep) {
		...
		case FILEHANDLING:
			break;
		case GENERIC:
			GenericKeywords genKeywords = GenericKeywords.getInstance();  
		...
		case WEBUI:
			...
			WebUI ui_keywords= WebUI.getInstance();// new WebUI();
