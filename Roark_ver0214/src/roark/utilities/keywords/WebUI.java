...
public class WebUI {
	...
	public static synchronized WebUI getInstance(){
	  if (instance == null){
	  instance = new WebUI();
	  }
	  return instance;
	...
	private WebElement findElementByXpath(String xpath){
		...
		try{
			if(this.isElementPresent( xpath)){
				...
				logger.error("targetObject is set to ::"+targetObject );
			}else{
				targetObject= null;
				...
				if(testcaseStep.getTestDataValue().trim().equalsIgnoreCase("TESTDATA_NOTFOUND")==false){
					try{
						...
						if(actualValue.trim().equals(testcaseStep.getTestDataValue())){
							...
							exitCode=5;
							testcaseStep.setErrDescription("Actual Value is not equal with expected value. \n Actual Value::"+actualValue +
	...
	public int storeAttributeValueFromElement(TestcaseStep testcaseStep, TestSuite testSuite, String attributeName) {
		...
		try{
			...
			if(targetElement!=null){
				try{
					String textFromElement = targetElement.getAttribute(attributeName).trim();
					if(testSuite.getRunTimeData().get(testcaseStep.getTestcaseID())==null){
						Map<String, String>runtimeDatarecord = new HashMap<String, String>();
						runtimeDatarecord.put(testcaseStep.getTestDataName(), textFromElement);
						testSuite.getRunTimeData().put(testcaseStep.getTestcaseID(), runtimeDatarecord);
